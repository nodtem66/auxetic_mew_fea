...
def zero_degrees_printing(B=B, pharse_offset=0, speed=CTS):
...
def sixty_degrees_printing_A(B, phase_offset=0, speed=CTS):
...
def sixty_degrees_printing_B(B, phase_offset=0, speed=CTS):
  f.write(f'; START Printing +60deg\n')
  t = PrintTransformation(f)
  t.set_rotate_angle(60)
...
with open(filename, 'w') as f:
  ...
  for s in range(number_scaffold):
    ...
    for layer in range(n):
      ...
      zero_degrees_printing(b, phase_offset, speed)
      sixty_degrees_printing_A(b, phase_offset, speed)
      relocate_A()
      sixty_degrees_printing_B(b, phase_offset, speed)
