...
filename = f'arrowhead-diag_nx{X_repetitions}ny{Y_repetitions}nL{layers}_cts{CTS}_{today_str}.gcode'
...
def main():
  with open(filename, 'w') as f:
    f.write(f'; Auxetic arrowhead with diagonal direction\n')
    ...
    n = layers
    half_y_repetitons = round(Y_repetitions/2)
    for l in range(n):
      if l == 0:
      ...
      for i in range(X_repetitions):
        for j in range(half_y_repetitons):
          f.write(f'G0 X{dAx} Y{dAy} F{CTS}\n')
          f.write(f'G0 X{-dBx} Y{-dBy} F{CTS}\n')
          f.write(f'G0 X{-dAx} Y{dAy} F{CTS}\n')
          f.write(f'G0 X{dBx} Y{-dBy} F{CTS}\n')
        for j in range(half_y_repetitons):
          f.write(f'G0 X{dBx} Y{dBy} F{CTS}\n')
